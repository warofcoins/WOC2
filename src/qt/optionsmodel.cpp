...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nAnonymizeWOC2Amount"))
        settings.setValue("nAnonymizeWOC2Amount", 1000);

    nDarksendRounds = settings.value("nDarksendRounds").toLongLong();
    nAnonymizeWOC2Amount = settings.value("nAnonymizeWOC2Amount").toLongLong();
    ...
    if (settings.contains("nAnonymizeWOC2Amount"))
        SoftSetArg("-anonymizewarofcoins2amount", settings.value("nAnonymizeWOC2Amount").toString().toStdString());
}

int OptionsModel::rowCount(const QModelIndex & parent) const
{
    return WarofCoins2IDRowCount;
}
...
bool OptionsModel::setData(const QModelIndex & index, const QVariant & value, int role)
{
    ...
    if(role == Qt::EditRole)
    {
        ...
        case DarksendRounds:
            ...
            break;
        case anonymizeWOC2Amount:
            nAnonymizeWOC2Amount = value.toInt();
            settings.setValue("nAnonymizeWOC2Amount", nAnonymizeWOC2Amount);
            emit anonymizeWOC2AmountChanged(nAnonymizeWOC2Amount);
            break;
        default:
            break;
        }
