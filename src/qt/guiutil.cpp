...
bool parseBitcoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    ...
    if(uri.scheme() != QString("warofcoins2"))
        return false;
...
{
    // Convert warofcoins2:// to warofcoins2:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("warofcoins2://"))
    {
        uri.replace(0, 12, "warofcoins2:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "WOC2.lnk";
}
...
{
    return GetAutostartDir() / "warofcoins2.desktop";
}

bool GetStartOnSystemStartup()
{
    boost::filesystem::ifstream warofcoins2File(GetAutostartFilePath());
    if (!warofcoins2File.good())
        return false;
    // Scan through file for "Hidden=true":
    std::string line;
    while (!warofcoins2File.eof())
    {
        getline(warofcoins2File, line);
        if (line.find("Hidden") != std::string::npos &&
            line.find("true") != std::string::npos)
            return false;
    }
    warofcoins2File.close();
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        boost::filesystem::ofstream warofcoins2File(GetAutostartFilePath(), std::ios_base::out|std::ios_base::trunc);
        if (!warofcoins2File.good())
            return false;
        // Write a bitcoin.desktop file to the autostart directory:
        warofcoins2File << "[Desktop Entry]\n";
        warofcoins2File << "Type=Application\n";
        warofcoins2File << "Name=WOC2\n";
        warofcoins2File << "Exec=" << pszExePath << " -min\n";
        warofcoins2File << "Terminal=false\n";
        warofcoins2File << "Hidden=false\n";
        warofcoins2File.close();
    }
    return true;
}
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("WOC2-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  warofcoins2-qt [" + tr("command-line options") + "]                     " + "\n";

    coreWarofCoins2s = QString::fromStdString(HelpMessage());

    uiWarofCoins2s = tr("UI options") + ":\n" +
        "  -lang=<lang>           " + tr("Set language, for example \"de_DE\" (default: system locale)") + "\n" +
        "  -min                   " + tr("Start minimized") + "\n" +
        "  -splash                " + tr("Show splash screen on startup (default: 1)") + "\n";

    setWindowTitle(tr("WOC2-Qt"));
    setTextFormat(Qt::PlainText);
    // setMinimumWidth is ignored for QMessageBox so put in non-breaking spaces to make it wider.
    setText(header + QString(QChar(0x2003)).repeated(50));
    setDetailedText(coreWarofCoins2s + "\n" + uiWarofCoins2s);
}
