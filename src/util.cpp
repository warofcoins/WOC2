...
#else
    const char* pszModule = "warofcoins2";
#endif
    if (pex)
        return strprintf(
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\WOC2
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\WOC2
    // Mac: ~/Library/Application Support/WOC2
    // Unix: ~/.WOC2
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "WOC2";
#else
    fs::path pathRet;
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "WOC2.conf"));
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "warofcoins2d.pid"));
